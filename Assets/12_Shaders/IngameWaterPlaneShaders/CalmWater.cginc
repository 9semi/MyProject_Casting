...
fixed4 frag( v2f i ) : SV_Target
{
	...
	// Atten
		...
		//UNITY_LIGHT_COORDS(atten, i, i.worldPos.xyz)
	#endif
	...
	// A 	= Depth
	// =======================================================================
	half4 refraction		= UNITY_SAMPLE_SCREENSPACE_TEXTURE(_GrabTexture, GrabUV.xy / GrabUV.w);
	half4 cleanRefraction  	= UNITY_SAMPLE_SCREENSPACE_TEXTURE(_GrabTexture, i.GrabUV.xy / i.GrabUV.w);
	
	//Depth Texture Clean
	float sceneZ 	= texDepth(i.DepthUV);
	//Depth Texture Distorted
	float DistZ 	= texDepth(DepthUV);
	...
	float dst = 1 - (i.DepthUV.z / 100);
	alpha = saturate(alpha) * dst;
	...
	fixed4 c;
		...
		float3 clean = lerp(cleanRefraction.rgb,diff, saturate(alpha) );
		c.rgb = clean;

		UNITY_APPLY_FOG(i.fogCoord, c);

		c.rgb = lerp(clean, c.rgb, saturate(alpha) * dst);
